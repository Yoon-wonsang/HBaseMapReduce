...
public class AverageScoreJoinReducer extends TableReducer<CustomKey, Text, ImmutableBytesWritable> {
    @Override
    protected void reduce(CustomKey key, Iterable<Text> values, Context context)
        ...
        if (grade != null) {
            ...
            writeResult(context, key.getYear(), midtermFinalSum, count, grade, midtermAverage, finalAverage, key.getStudentId());
        }
    }

    private void writeResult(Context context, String year, float midtermFinalSum, int count, String grade,
                             float midtermAverage, float finalAverage, String studentId)
            throws IOException, InterruptedException {
        ...
        context.write(new ImmutableBytesWritable(Bytes.toBytes(studentId)), put);
    }
}
